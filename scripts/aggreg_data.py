...
if os.path.isfile(snakemake.output[0]):
    ...
    pickle_sub = pd.read_pickle(snakemake.input[0])
    pickle_concat = pd.concat([pickle_df,pickle_sub])
    pickle_concat.to_pickle(snakmake.output[0])
else:
...
def visualize_subj(pickle):
    ...
    for sub in subjects:
        ...
        plt.savefig(os.path.join('output',sub+'boxplot.png'))
        ...
        plt.savefig(os.path.join('output',sub+'ap.png'))
